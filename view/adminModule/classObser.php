...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['selected_date'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($updateStmt->execute()) {
            ...
            $recipient = 'gonzalvoglenn123@gmail.com';
            $subject = 'Classroom Observation for the ' . $semester . ' Semester of the ' . $academic_year . ' academic year';
            $body = "
            Dear Faculty Members,

We are pleased to inform you that classroom observations for the $semester Semester of the $academic_year academic year are now open. You may begin selecting your preferred dates for the classroom observations at your earliest convenience.
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).ready(function () {
            ...
            $('#view-date-select').change(function () {
        ...
        function createReservationTable(bookings = []) {
            ...
            if (bookings && bookings.length > 0) {
                bookings.forEach(booking => {
                    ...
                    const status = booking.evaluation_status;

                    for (let hour = startHour; hour < endHour; hour++) {
                        const slotKey = `${hour}-${date}-${slotNumber}`;
                        bookedSlots[slotKey] = booking;  // Store booking information in bookedSlots object
                    }
                });
            }
                    ...
                    if (bookedSlots[slotKey1]) {
                        ...
                        if (booking.evaluation_status == 1) {
                            ...
                            cell1.addClass('py-3 booked-slot').css({
                                ...
                                'cursor': 'pointer'
                            }).html(`${booking.name}<br>${booking.room}`).data('slotKey', slotKey1);
                        }
                    } else {
                    ...
                    if (bookedSlots[slotKey2]) {
                        ...
                        if (booking.evaluation_status == 1) {
                            ...
                            cell2.addClass('py-3 booked-slot').css({
                                ...
                                'background': '#f2b2b2', // Color for regular booked slots
                                'cursor': 'pointer'
                            }).html(`${booking.name}<br>${booking.room}`).data('slotKey', slotKey2);
                        }
                    } else {
        ...
        $(document).on('click', '.booked-slot', function () {
            ...
            if (booking) {
                console.log('Booking clicked:', booking); // Log the booking to verify the data

                const today = new Date().toISOString().split('T')[0];

                // Check if selectedDate exists and is valid
                if (booking.selected_date) {
                    const bookingDateObj = new Date(booking.selected_date);
                    ...
                    if (!isNaN(bookingDateObj.getTime())) {
                        ...
                        if (bookingDate === today) {
                            if (!booking.isEvaluated) {
                                const dateConvert = new Date(booking.selected_date);
                                const options = { year: 'numeric', month: 'long', day: 'numeric' };
                                const formattedDateText = dateConvert.toLocaleDateString('en-US', options);

                                $('#booking-name').text(booking.name);
                                $('#toFacultyID').val(booking.faculty_Id);
                        ...
                        console.error('Invalid booking date:', booking.selected_date);
                    }
                } else {
                    console.error('booking.selected_date is undefined or invalid');
                }
            }
        });
        ...
        function checkAndAutoBook() {
            ...
            facultySchedules.forEach(faculty => {
                // Check if there is a booking within the next 30 days
                if (!hasBookingWithinMonth(selectedDate, faculty.name)) {
                    autoBookFullRange(selectedDate, faculty.name);
                }
            });
        }

        function hasBookingWithinMonth(selectedDate, facultyName) {
            const monthDuration = 30; // Days to check for bookings
            const endDate = new Date(selectedDate);
            endDate.setDate(selectedDate.getDate() + monthDuration);

            for (let key in bookedSlots) {
                const bookingDate = new Date(parseInt(key.split('-')[1]));
                if (bookingDate >= selectedDate && bookingDate <= endDate && bookedSlots[key].name === facultyName) {
                    return true; // Found a booking within the next 30 days
                }
            }
        ...
        function hasBookingForSelectedDate(selectedDate, facultyName) {
            for (let key in bookedSlots) {
                const bookingDate = new Date(parseInt(key.split('-')[1]));
                // Check if the date matches and if the faculty name matches
        ...
        function autoBookFullRange(selectedDate, facultyName) {
            if (isBookedThisWeek(facultyName, selectedDate)) {
                console.log(`Already booked this week for ${facultyName}.`);
                return; // Stop booking if already booked this week
            }

            const faculty = facultySchedules.find(f => f.name === facultyName);
            if (!faculty) {
                return; // If faculty does not exist, exit the function
            }
                ...
                for (const [day, slots] of Object.entries(faculty.schedule)) {
                    if (day === dayOfWeek) {
                        for (const slot of slots) {
                            const { start, end } = slot; // Assume start and end are hours (e.g., 10 for 10 AM)

                            // Check availability for both slots
                            let canBookSlot1 = true;
                            let canBookSlot2 = true;

                            // Check for slot 1 availability
                            for (let hour = start; hour < end; hour++) {
                                const slotKey1 = `${hour}-${currentDate.getTime()}-1`;
                                if (bookedSlots[slotKey1]) {
                                    canBookSlot1 = false; // Slot 1 not available
                                    break;
                                }
                            }

                            // Check for slot 2 availability
                            for (let hour = start; hour < end; hour++) {
                                const slotKey2 = `${hour}-${currentDate.getTime()}-2`;
                                if (bookedSlots[slotKey2]) {
                                    canBookSlot2 = false; // Slot 2 not available
                                    break;
                                }
                            }

                            // Book Slot 1 if available
                            if (canBookSlot1) {
                                const bookingID1 = `${facultyName}-${currentDate.toISOString().split("T")[0]}-${start}-${end}-1`;
                                ...
                                for (let hour = start; hour < end; hour++) {
                                    ...
                                    // Save the booking only once for slot 1
                                    if (hour === start) {
                                        $.ajax({
                                            type: "POST",
                                            url: "saveBooking.php",
                                            ...
                                            error: function (xhr, status, error) {
                                                console.error("Error saving booking for slot 1:", error);
                                            }
                                        });
                                    }
                                }
                            ...
                            // Book Slot 2 if available
                            if (canBookSlot2) {
                                const bookingID2 = `${facultyName}-${currentDate.toISOString().split("T")[0]}-${start}-${end}-2`;
                                ...
                                for (let hour = start; hour < end; hour++) {
                                    ...
                                    // Save the booking only once for slot 2
                                    if (hour === start) {
                                        $.ajax({
                                            type: "POST",
                                            url: "saveBooking.php",
                                            ...
                                            error: function (xhr, status, error) {
                                                console.error("Error saving booking for slot 2:", error);
                                            }
                                        });
                                    }
                                }
