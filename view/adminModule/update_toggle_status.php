...
$query = "SELECT semester, academic_year FROM academic_year_semester WHERE id = 3";
...
if ($result->num_rows > 0) {
    ...
    // Check if semester and academic_year are valid (not null and not empty)
    $switch_state = 0; // Default to off (0)
    if (!empty($row['semester']) && !empty($row['academic_year'])) {
        $switch_state = 1; // Turn on if both are non-empty
    }

    // Return the semester, academic_year, and switch state as JSON
    echo json_encode([
        'semester' => $row['semester'],
        'academic_year' => $row['academic_year'],
        'switch_state' => $switch_state // Return the computed switch state
    ]);
} else {
    // Return an error or default values if no data is found
    echo json_encode([
        'semester' => null,
        'academic_year' => null,
        'switch_state' => 0 // Default to off if no data found
    ]);
}
